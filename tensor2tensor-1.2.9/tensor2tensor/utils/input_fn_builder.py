...
def build_input_fn(mode,
  ...
  def input_fn():
    ...
    if problem_instance.has_inputs:
      ...
      if "firstP" in feature_map:
        feature_map["firstP"].set_shape([None, None, None, None])
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
      ...
      if problem_instance.has_inputs:
        ...
        if "firstP" in feature_map:
          feature_map["firstP"]._shape = tf.TensorShape([None, None, None, None])  # pylint: disable=protected-access
...
def features_for_problem(problem_instance,
  ...
  if problem_instance.has_inputs:
    while len(feature_map["inputs"].get_shape()) != 4:
      ...
      if "firstP" in feature_map:
        while len(feature_map["firstP"].get_shape()) != 4:
          feature_map["firstP"] = tf.expand_dims(feature_map["firstP"], axis=-1)
