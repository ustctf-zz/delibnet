...
class T2TModel(object):
  ...
  def _beam_decode_slow(self, features, decode_length, beam_size, top_beams,
    ...
    if self.has_input:
      ...
      firstP_old = features["firstP"]
      features["firstP"] = tf.expand_dims(features["firstP"], 1)
      if len(features["firstP"].shape) < 5:
        features["firstP"] = tf.expand_dims(features["firstP"], 4)
      # Expand the firstP in to the beam size.
      features["firstP"] = tf.tile(features["firstP"], [1, beam_size, 1, 1, 1])
      z = tf.shape(features["firstP"])
      features["firstP"] = tf.reshape(features["firstP"],
                                      [z[0] * z[1], z[2], z[3], z[4]])
  ...
  def _slow_greedy_infer(self, features, decode_length):
    ...
    if not features:
      features = {}
    inputs_old, firstP_old = None, None
    if "inputs" in features and len(features["inputs"].shape) < 4:
      inputs_old = features["inputs"]
      features["inputs"] = tf.expand_dims(features["inputs"], 2)
    if "firstP" in features and len(features["firstP"].shape) < 4:
      firstP_old = features["firstP"]
      features["firstP"] = tf.expand_dims(features["firstP"], 2)
  ...
  def model_fn(self, features, skip=False, force_full_predict=False):
    ...
    for key, input_modality in six.iteritems(self._problem_hparams.input_modality):
      if "firstP" in key:
        continue
      previous_modalities = [
    ...
    with tf.variable_scope(target_modality.name, reuse=True):
      transformed_features["firstP"] = target_modality.targets_bottom_sharded(
          sharded_features["firstP"], dp)

    # Allows later access to pre-embedding raw targets.
    transformed_features["firstP_raw"] = sharded_features["firstP"]
