...
model_prefix=delibnet_fromRL26.67_adam0.01_dr0.05
...
ids=$(ls ${RPATH}/${model_prefix} | grep "model\.ckpt-[0-9]*.index" | grep -o "[0-9]*")
...
for iter in ${ids}; do
    ...
    mkdir ${model_dir}
    cp ${RPATH}/${model_prefix}/model.ckpt-${iter}* ${model_dir}
